export class TimeLine {
  ...
  constructor(canvas: HTMLCanvasElement) {
    $effect(() => {
      const _ = this.position + this.velocity_position + this.target_position
    ...
    $effect(() => {window.createImageBitmap(this.image).then((image) => {this.#image_element = image})});
  ...
  draw() {
    ...
    this.ctx.drawImage(
      ...
      // 3, 1,
      -this.position * this.scale + this.canvas.width / 2, 40,
      this.scale * this.image.width, this.canvas.height);
